...
type DashMsg = { type: 'beacons'; beacons: Beacon[]; stats: { total: number; alive: number; packages: number }; now: number };
...
export default function BeaconsClient({
  initialBeacons,
  initialNow,
}: {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (msg.type === 'beacons') { setBeacons(msg.beacons); setNow(msg.now); }
      } catch { /* ignore */ }
    };
    return () => ws.close();
  }, []);

  const active = beacons.filter(b => b.kill === 0);
  const archived = beacons.filter(b => b.kill === 1);
