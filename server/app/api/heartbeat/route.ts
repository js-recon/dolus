...
export async function POST(req: Request) {
  ...
  const beacon = db.prepare('SELECT id, kill FROM beacons WHERE beacon_id = ?').get(body.beacon_id) as { id: number; kill: number } | undefined;
  ...
  return NextResponse.json({ ok: true, kill: beacon.kill === 1 });
}
